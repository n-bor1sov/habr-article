...
3. **Формирование запроса в модель:**

На основе извлечённых данных формируется итоговый запрос вида
```
{СУЩНОСТИ}
```
...
![LightRAG иллюстрация созданного графа знаний](LightRAG_graph_image.png)  
...
- **Построение пути расслеждений**  
  ...
  Получив топ k лучших путей, мы формируем контекст из их сущностей и привязанных к ним чанков.

## Результаты

- **Дизайн экспериментов**  
  Нашей основной задачей было тестирование данных алгоритмов в домене X5.
  Мы проводили эксперименты на двух базах знаний чтобы оценить применимость алгоритмов к разным видам данных
