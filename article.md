...
<img src="LightRAG_graph_image.svg" alt="RAG иллюстрация" style="width:50%; height:auto;"> 
...
- **Дизайн экспериментов**  
  ...
  - Первая база данных содержала около 500 рабочих инструкций, что позволило нам установить бэйзлайн для процесса ретрива: RAG.  
    Для генерации мы использовали комбинацию RAG с Reranker и Chaining, что дало нам возможность оценить качество ответов.
  - Вторая база данных состояла из примерно 1500 записей в формате вопрос-ответ, что предоставило нам возможность протестировать алгоритмы в более сложных условиях. Здесь бэйзлайн для ретрива был основан на RAG с использованием симметричного поиска, что позволило нам сравнить результаты с предыдущими экспериментами.
...
![метрики](метрики.png)
## Заключение
- Подходы показали state-of-the-art метрики на задачах ретрива и генерации.
- Использование графовых RAG может показать сильный прирост метрик в задачах, где требуется не находить конкретный документ или инструкцию, а комбинировать ответ на вопрос из различных элементов, находящихся в разных документах.
